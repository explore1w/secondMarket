...
public class Main extends Frame{
     ...
     * 每个格子的边长
     */
    private static int size=50;//方格的长和宽
    /*
     ...
     */
    public Main(int length_of_side, int row_num, int column_num, int start_x, int start_y, int end_x, int end_y) {
        size=length_of_side;
    ...
    public void paint(Graphics g) {
        drawSquare(g);
        g.drawString("共找到路径："+rightPaths.size()+"条", WIDTH/4-100, 80);
    ...
    private void drawSquare(Graphics g){
        ...
        for (int i = size; i <= squareWidth; i+=size) {
            for (int j = size; j <= squareHeight; j+=size) {
                ...
                g.drawString("("+j+","+i+")", i, j);
            }
        }
        //画出起点和终点
        g.setColor(Color.blue);
        g.setFont(new Font("", Font.BOLD, 40));
        g.drawString("起点("+starPoint.x+","+starPoint.y+")", starPoint.x-20, starPoint.y+5-40);//起点
        g.drawString("\u265E", starPoint.x-10, starPoint.y+5);//起点
        g.drawString("终点("+endPoint.x+","+endPoint.y+")", endPoint.x-20, endPoint.y+5-40);//终点
...
//		System.out.println("退出循环tryPints长度为："+tryPoints.size());
    ...
    String path;
    private void tryWay(){
        if(nextPoint.y>=100&&nextPoint.x>=100&&nextPoint.y<=squareHeight&&nextPoint.x<=endPoint.x){//下一个点nexPoint未越界,下一个点不能到达终点的右侧
...
//				System.out.println("jump");
    ...
    public static void main(String[] args) {
        ...
        new Main(100,20, 20, 100, 100, 500,600);
    }
/**
* 借鉴！
 */
